...
/*   Updated: 2024/03/26 14:17:18 by rachou           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	target_node_a(t_stack *a, t_stack *b)//Fonction qui me permet de trouver les noeuds target dans ma stack 'a' pour les noeuds de la stack 'b'.
{
    ...
    while (b)
    {
        ...
        if (target == LONG_MAX)
            b->current_target_node = find_max(a);//Dans le cas où mon noeud 'b' ne trouve pas un noeud target le plus proche petit, j'irai trouver le plus proche grand à la place.
        else
